...
void publish_unix(const fs::path& cache_base, const std::string& pack_id,
    ...
    const fs::path hook = cache_base / "path.sh";
    {
        ...
        hs << "# Auto-generated by Retro — do not edit; re-run ensure to refresh.\n"
    ...
    block << begin << '\n'
          << "# Managed by Retro / RetroPorting-Toolchains install — remove via uninstall.sh\n"
...
bool publish_toolchain_user_env(const Paths& paths, const std::string& pack_id,
    ...
    if (using_custom_root(paths)) {
        if (message)
            *message = "toolchain PATH: skipped (custom Retro folder — no shell/registry "
